...
/* -----------------------------------------------------------------------------
   ...
   CRTICEPGM STMF('/www/IceBreak-Samples/unittest.rpgle') SVRID(samples)
...
dcl-proc main;
    ...
    // required output:
    if json_getStr(pOutput:'message') <>
        'My name is John';
        fail(errCount: 'Invalid message': pInput: pOutPut);
    endif;

    logText( 
        json_GetStr(pinput:'action') + 
        ' Completed with : ' +  %char(errCount) + ' Errors'
    );
